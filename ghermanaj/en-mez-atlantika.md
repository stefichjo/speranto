...
| | | | | |
|-|-|-|-|-|
| ɴᴇᴀʀ<small>##</small>   | `ɪjɐ` | |     |           |
| sǫᴜᴀʀᴇ<small>##</small> | `εjɐ` | |     |           |
| | | | | |
| ꜰᴏʀᴄᴇ<small>##</small>  | `oʉɐ`  | | ꜰᴏʀ<small>##</small> | `ɔʉɐ` |
| | | | | |
...
Notes:
* ꜰᴇᴇʟ `fɪɵᴸ` ← `fɪo` Wells
* ꜰᴀɪʟ `fεɵᴸ` ← `fæɤ` = ꜰᴏᴜʟ Wells
* ᴠᴇɪʟ `vεɵᴸ` ← `væɤ` = ᴠᴀʟ Wells
* ʀᴏʟʟ `rɔʉᴸ` ← `rɒʊ` Wells
* ᴅᴏʟᴇ `dɔʉᴸ` = ᴅᴏʟʟ
  * ᴘᴏʟᴇ `pɔʉᴸ` ≠ ᴘᴏʟʜᴀʀ `pɐʉlə`
  * Hypercorrection + Split
    * ʀᴏʟʟ `rɔʉᴸ`
      * ʀᴏʟʟᴇʀ `rɐʉləʳ` → `rɔʉləʳ`
      * ʀᴏʟʟɪɴɢ `rɐʉlɪn` → `rɔʉlɪn`
    * ɢᴏᴀʟ `gɔʉᴸ`
      * ɢᴏᴀʟᴇᴇ `gɐʉləj` → `gɔʉləj`
* ᴛᴏᴏʟ `tʉːᴸ` ← `toʊ~tʊu` Wells
* ᴛᴡᴏ `təʉ` ← `tʊʉ` Wells
* ᴠᴀʟ `væɵ` ← `væɤ` = ᴠᴀʟ Wells
* ᴅʀᴇss `ε` ← `e` Wells
* ʙᴇᴅ `bεjd` ~ `bejd`
* ʙᴀᴅ `bæjd` ~ `bεjd` ← `εːj` Wells

<!-- TODO people `o` -->
<!-- TODO awful, musical `oː` -->
...
|-|-|-|-|-|-|-|-|
| ᴘʀɪᴄᴇ  | `ɑjʳ` | | ᴍᴏᴜᴛʜ   | `æʉʳ`    | |         |       |
| ᴄʜᴏɪᴄᴇ | `oj`  | | ᴛʜᴏᴜɢʜᴛ | `oː~oʉᴸ` | | sᴀᴡ     | `ɔːʳ` |
| | | | | | | | |

Notes:
* `ɑjʳ` = `ɑj~ɑːʳ`
* `æʉʳ` = `æʉ ~ æəʳ ~ æːʳ ~ æjɐ`
* ꜰᴀʟʟ `foʉᴸ` ← `foʉᴸ~foːᴸ` Wells
  * ꜰᴀʟʟɪɴɢ `foːlɪŋ`
* ꜰᴏᴜʟ `fæɵᴸ` ← `fæɤ` Wells
